...
- **Recognition passes two gates**: at the first, a set of letters absent from Arabic (ٹ پ چ ژ گ ی among them) separates Urdu and Persian from Arabic (ی is the Persian yeh, a different codepoint from Arabic's ي); at the second, a few letters only Urdu writes tell Urdu apart from Persian: Urdu gets Nastaliq, Persian gets Vazirmatn. That first gate once omitted ی itself: every letter of بیاض except that one falls inside ordinary Arabic, so the product's own name dropped into the Arabic tier and was set in Naskh. Adding the single codepoint took the name out of the Arabic tier. On its own, بیاض is still read as Persian and shown in Vazirmatn: a lone word built only from shared letters gives no sign of Urdu.
